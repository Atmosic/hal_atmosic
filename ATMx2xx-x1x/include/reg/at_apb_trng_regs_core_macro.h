 ...
 * @details this field is not used
 * @{
 */
 ...
 * @details when radio is first coming back from power down mode, generate automatically (without software intervension) a new random number; this core will assert the force_radio_into_trng_mode, wait 50usec, and then start the bit gathering process
 * @{
 */
 ...
 * @details the hardware fsm, when generating the random number, will force the radio to disable the RF input and to maximize all gain stages; set this bit to block; if blocked, radio will operate normally but trng will still run
 * @{
 */
 ...
 * @details specifies how long to wait for radio to warmup in clk_mpc cycles before starting the random number generation process; a value of 0 is not supported
 * @{
 */
 ...
 * @details generation of random number is in progress
 * @{
 */
...
#define TRNG_STATUS__READ                                           0x00000001U
 ...
 * @details random number generation completed and random number is available
 * @{
 */
 ...
 * @details random number generation timed out and random number is not available
 * @{
 */
 ...
 * @brief the random number definitions.
 * @{
 */
