...
{
    if (CMSDK_AT_DMA->CHAN2_INTERRUPT_STATUS) {
	chan2_cb(chan2_ctx);
...
{
    if (CMSDK_AT_DMA->CHAN3_INTERRUPT_STATUS) {
	chan3_cb(chan3_ctx);
    ...
    uint16_t size)
{
    ...
    while (!CMSDK_AT_DMA->CHAN1_INTERRUPT_STATUS) {
#ifdef DEBUG_DMA
	dma_wait_cnt++;
#endif
	YIELD();
...
{
    if (CMSDK_AT_DMA->CHAN1_STATUS || CMSDK_AT_DMA->CHAN2_STATUS ||
