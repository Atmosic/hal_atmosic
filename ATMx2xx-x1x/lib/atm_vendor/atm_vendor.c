...
#else
static uint8_t const AG_READY_EVT[] = {HCI_EVT_MSG_TYPE, HCI_EVT_VENDOR, 0x06,
...
{
    hcievent_buffer[0] = HCI_EVT_MSG_TYPE;
...
static void gadc_result(float result, int16_t raw, struct gadc_cal_s cal,
...
#ifdef CFG_VND_OTP_PUSH
static void vendor_otppush_handler(uint8_t *buf)
{
...
{
    // check hci packet and event type
    if ((*bufptr != HCI_EVT_MSG_TYPE) || (*(bufptr + 1) != HCI_EVT_CMD_CPM)) {
	return false;
    }
...
static rep_vec_err_t vendor_uart_event(uint32_t buf_size, uint8_t *buf)
{
	...
	(buf_size == HCI_TRANSPORT_HDR_LEN) && (*buf == HCI_CMD_MSG_TYPE)) {
...
{
    if ((*size == BASIC_HCI_EVT_HD_LEN) && (**bufptr == HCI_EVT_MSG_TYPE) &&
